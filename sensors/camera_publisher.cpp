...
public:
  ...
  {
    width_ = static_cast<uint32_t>(this->declare_parameter<int64_t>("width", 640));
    height_ = static_cast<uint32_t>(this->declare_parameter<int64_t>("height", 480));
...
protected:
  void on_tick(uint64_t seq, const rclcpp::Time & stamp) override
  {
    ...
    // Diagonal gradient that scrolls with the sequence number; small deterministic
    // per-pixel noise. Reproducible for a given seed.
    const int phase = static_cast<int>(seq) & 0xFF;
    for (uint32_t y = 0; y < height_; ++y) {
      for (uint32_t x = 0; x < width_; ++x) {
        const size_t off = static_cast<size_t>(y) * msg.step + static_cast<size_t>(x) * 3;
        const int n = static_cast<int>(noise(noise_amp_ * 0.5));
